...
public class MapsActivityTest {
    ...
    private IdlingResourceWrapper idlingResource;

    @Before
    public void setUp() {
        activityRule.getScenario().onActivity(activity -> {
            idlingResource = new IdlingResourceWrapper();
            activity.getMap().setOnCameraIdleListener(idlingResource);
    ...
    @Test
    public void testGeoJsonLoaded_populatesBuildingsMap() {
        // Wait for the map to be ready and GeoJSON to be loaded.
        activityRule.getScenario().onActivity(activity -> {
            // Check that the buildingsMap is not empty
            assertFalse("buildingsMap should not be empty after GeoJSON is loaded", MapsActivity.buildingsMap.isEmpty());

            // Check for a specific, known building from the GeoJSON file
            Building hallBuilding = MapsActivity.buildingsMap.get("way/22080570");
            assertNotNull("Hall Building should be in the map", hallBuilding);
            assertEquals("H - Henry F. Hall Building", hallBuilding.name);
        });
    }
}
